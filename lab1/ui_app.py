...
class BFSApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("BFS Visualization (Breadth-First Search)")
    ...
    def _build_ui(self):
        ...
        # Editing vertices
        ttk.Label(control_frame, text="Edit vertices", font=HEADER_FONT).pack(pady=PADY_SECTION, anchor=tk.W)
        
        node_row = ttk.Frame(control_frame)
        node_row.pack(fill=tk.X, pady=PADY_FIELD)
        ttk.Label(node_row, text="Node ID:").pack(side=tk.LEFT)
        self.node_id_var = tk.StringVar()
        ttk.Entry(node_row, textvariable=self.node_id_var, width=6).pack(side=tk.LEFT, padx=(2, 10))
        
        node_btn_frame = ttk.Frame(control_frame)
        node_btn_frame.pack(fill=tk.X, pady=(4, 2))
        ttk.Button(node_btn_frame, text="Add Node", command=self.add_node_ui).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(0, 2))
        ttk.Button(node_btn_frame, text="Remove Node", command=self.remove_node_ui).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(2, 0))
        ...
        # Search parameters
        ttk.Label(control_frame, text="BFS parameters", font=HEADER_FONT).pack(pady=PADY_SECTION, anchor=tk.W)
    ...
    def _get_node_input(self):
        try:
            return int(self.node_id_var.get())
        except ValueError:
            messagebox.showerror("Error", "Enter numeric vertex ID")
            return None

    def add_node_ui(self):
        u = self._get_node_input()
        if u is not None:
            if self.logic.add_custom_node(u):
                self.draw_graph()
            else:
                messagebox.showwarning("Warning", "Vertex already exists.")

    def remove_node_ui(self):
        u = self._get_node_input()
        if u is not None:
            if self.logic.remove_custom_node(u):
                self.draw_graph()
            else:
                messagebox.showwarning("Warning", "Vertex does not exist.")
    ...
    def replace_with_arc_ui(self):
        ...
        if u is not None and v is not None:
            if self.logic.replace_edge_with_arc(u, v):
                self.graph_type_var.set("Directed graph")
                self.draw_graph()
            else:
    ...
    def init_search(self):
        ...
        self.search_generator = self.logic.bfs_generator(start, target, self.active_order)
        
        self.current_state = None
        self.algo_elapsed_ms = 0.0
        self.update_results_ui("Search initialized", "BFS (Breadth-First)", 0, "-", "-")
        self.draw_graph()
    ...
    def show_result_popup(self, state, elapsed_time):
        ...
        algo_name = "BFS (Breadth-First Search)"
    ...
    def update_ui_from_state(self, state):
        ...
        path_str = " -> ".join(map(str, state["path"])) if state["path"] else "-"
        self.update_results_ui(status_text, "BFS", state["expanded"], path_str, elapsed_time)
